...
def c_obtener_usuario_por_email(db, entrada):
    # Validaciones inicio
    usuario=db.query(Usuario).filter(Usuario.email==entrada).first()
    if usuario is None:
        raise HTTPException(
...
def crear_usuario(db, entrada:UsuarioCreate):
    # Validaciones inicio
    validacion = db.query(Usuario).filter(Usuario.email==entrada.email).first()
    if validacion is not None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="El email ya existe")
    # Validaciones fin
    try:
        usuario = Usuario(email=entrada.email, password=bcrypt_context.hash(entrada.password))
        db.add(usuario)
        db.commit()
        db.refresh(usuario)
        return True
...
def c_login(db, entrada:UsuarioSesion):
    # Validaciones inicio
    usuario=db.query(Usuario).filter(Usuario.email==entrada.email).first()
    if usuario is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="El email no existe")
    # Validaciones fin
    try:
        query_result = db.query(Usuario).filter(Usuario.email==entrada.email).first()
        if query_result:
